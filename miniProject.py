...
class scraping:
		...
		try:
			...
			try:
				...
				if(str(sem).translate(None,string.punctuation+string.whitespace).lower()!= "semester5"):
					...
					print(Style.BRIGHT + Fore.GREEN + usnn + " "+ name)
	...
	def search(self):
		file_name = raw_input("Enter the name of file with proper extension: ")
		keyword = raw_input("Enter the string to search: ")
		...
		try:
			for x in range(1,ws.max_row):
				...
				if ws.cell(x,1).value.lower()==keyword:
					sflag=1
					for y in range(1,ws.max_column):
						if(ws.cell(1,y).value==None):
                		         		print " ", 
                				else:
                		 		       print ws.cell(1,y).value,
                			print("\n")
	...
	def delete_entry(self):
		file_name = raw_input("Enter the name of file with proper extension: ")
		keyword = raw_input("Enter the record you want to delete: ")
				...
				if ws.cell(x,1).value.lower()==keyword:
					...
					for y in range(1,ws.max_column):
						if(ws.cell(1,y).value==None):
                		 		       ...
                		 		       print "Content of "+ws.cell(1,y).value+" :",
                		 		       print(ws.cell(x,y).value)
                		 		       modify=raw_input("Modify it as: (press enter to skip or type 'done' to finish): ").lower()
